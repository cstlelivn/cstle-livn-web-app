...
app.get("/make-server-bcab437c/gallery/manage", authMiddleware, async (c) => {
  ...
  const { data: albums, error: albumError } = await supabase.from("gallery_albums").select("*").order("display_position", { ascending: true }).order("created_at", { ascending: false }).limit(200);
  ...
  return c.json({ albums: (albums ?? []).map((album: any) => ({ ...album, images: imagesByAlbum.get(String(album.id)) ?? [] })) });
});
...
app.put("/make-server-bcab437c/gallery/albums/:id", authMiddleware, async (c) => {
  ...
  const services = Array.isArray(body.services) ? [...new Set(body.services.map((item: unknown) => String(item).trim().slice(0, 60)).filter(Boolean))].slice(0, 16) : [];
  ...
  if (published) {
    ...
    const { data: publishImages, error: publishImageError } = await supabase.from("gallery_images").select("id, alt_text, published, is_active").eq("album_id", albumId).eq("published", true).eq("is_active", true);
    ...
    if (publishImages.some((image: any) => !String(image.alt_text ?? "").trim())) return c.json({ error: "Every published image needs useful alternative text" }, 400);
  }
  let coverUrl: string | null = null;
  if (coverImageId) {
    const { data: cover, error: coverError } = await supabase.from("gallery_images").select("id, url, thumbnail_url").eq("id", coverImageId).eq("album_id", albumId).maybeSingle();
  ...
  const { data, error } = await supabase.from("gallery_albums").update({
    public_title: title, public_slug: slug, description, project_type: projectType, services,
    location_label: locationLabel, status, cover_image_id: coverImageId, cover_url: coverUrl,
    display_position: Number.isFinite(Number(body.display_position)) ? Number(body.display_position) : 0,
    published, published_at: published ? new Date().toISOString() : null, updated_at: new Date().toISOString(),
...
app.put("/make-server-bcab437c/gallery/albums/:id/images", authMiddleware, async (c) => {
  ...
  for (let index = 0; index < images.length; index += 1) {
    ...
    const { error } = await supabase.from("gallery_images").update({
      ...
      alt_text: altText, stage, display_position: index, published: isPublished,
      is_active: image.is_active !== false, updated_at: new Date().toISOString(),
    }).eq("id", imageId).eq("album_id", albumId);
