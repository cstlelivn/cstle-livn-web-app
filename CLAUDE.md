...
- **Explicit product decision, overriding the previous entry's approach**:
  the user pushed back on the "due_date is exclusive" framing from the
  prior fix -- "the due date of a task is supposed to be the day the task
  is supposed to end, not the day after. That's the way we look at things
  here." Rather than keep working around the exclusive convention (as the
  previous entry did, adjusting only the Sunday-check's displayed date),
  `due_date`/`end_date` are now genuinely INCLUSIVE wherever this app
  *generates* a schedule: **the due/end date IS the last day of work.** A
  1-day task now gets `start_date === due_date` (previously due_date was
  the day after).
- **`applyTemplateToProject()`** (`src/app/src/features/projectTemplates/api.ts`)
  changed from `due = addWorkDays(start, duration)` (next-available-start,
  exclusive) to `due = addWorkDays(start, duration - 1)` (the actual last
  workday, inclusive) for both phases and tasks. The "where does the next
  phase/task start" advancement logic was updated to match (next start =
  one workday past the new inclusive end, tracked via a separate
  `lastActualEndDate`/`lastTaskDueDate` rather than reusing the old
  next-start `cursor` as if it were an end date). Verified with a
  standalone Node simulation before shipping: a 1-day task now gets
  `start === due`; a 3-day task starting Wed 2026-09-02 gets
  `due = 2026-09-04`, a true 3-calendar-day span.
- **`GanttChart.tsx`** updated to match: `getBarPosition()`'s
  `endOffset` gained a `+1` (a bar now visually covers through its due
  date, not up to it); the resize floor checks changed from `< 1` to
  `< 0` (start can now legally equal due for a 1-day task); `
  resolveEndDate()`'s Sunday check now tests the due/end date directly
  again (no more `-1` adjustment -- that was the previous entry's
  workaround for the old convention, now unnecessary and reverted);
  `offerSaveDurationToTemplate()` calls gained a matching `+1` so a
  resized task's saved-back `default_duration_days` stays correct.
- **`ProjectDetailsReal.tsx`**: the two `durationDays` display calculations
  (task detail duration badges) gained the same `+1` -- they compute
  "how many days does this task span" from `start_date`/`due_date`, which
  needed to move in lockstep with the new inclusive meaning. The Calendar
  view's drag-to-reschedule handler (`handleDrop`, same file) needed NO
  change -- it only ever preserves the existing start-to-due *gap* when
  shifting a task, which is convention-agnostic (the gap size is the same
  number regardless of which end is called inclusive).
- **Existing data is NOT touched.** Tasks/phases already in the database
  keep whatever `due_date`/`end_date` value they already have -- there is
  no way to know from this session whether a given historical value was
  generated by the old exclusive scheduler (and so is arguably "one day
  too late" under the new convention) or was manually date-picked by a
  person who already meant it as the true last day (and so is already
  correct). Retroactively shifting every historical task by -1 day would
  risk being wrong for exactly the tasks a human already set correctly.
  This only changes what happens **going forward**: new projects created
  from a template, and any task/phase whose date is set via a fresh
  Gantt drag/resize.
- `npx tsc --noEmit -p tsconfig.sync.json`, `npm run build`, and `npm test`
  (13/13) all pass. The scheduler math was verified with a standalone
  Node script (not just reasoned about) before shipping, same standard as
  the previous entry's NaN-comparator fix. **Not live-verified** -- same
  standing limitation as every other Gantt entry in this file (this
  session's browser can't reliably deliver drag gestures to the app). The
  user should create one new project from a template and confirm a task
  they know should be "1 day" actually shows `start === due`, then try
  dragging/resizing a bar in the Gantt and confirm the due date it lands
  on matches exactly where the bar visually stops.
